...
const { contactFromOihPerson } = require('../transformations/contactToOihPerson');
...
function processTrigger(msg, cfg, passedSnapshot, incomingMessageHeaders, tokenData) {
  ...
  return Swagger.execute(callParams).then(async (passedData) => {
    ...
    // Transform contacts to oih to contact data format
    if (newElement.data && 'contacts' in newElement.data) {
      let i;
      for (i in body.contacts) { // eslint-disable-line
        newElement.data.contacts[i] = transform(newElement.data.contacts[i], cfg, contactFromOihPerson);
      }
